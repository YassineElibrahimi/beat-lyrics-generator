...
"""
Explanation:
1. 'get_random_progression' queries the database for all progressions matching the genre and theme,
then picks one at random.

2. 'roman_to_chords' uses 'music21.roman.RomanNumeral' to parse a Roman numeral string (like "i") in the context of a key, and extracts the actual chord.
I assign a whole-note duration for now (chord_obj.duration.quarterLength = 4.0);
later we'll adjust based on tempo and style.

3. 'generate' combines both steps.
...
class ChordGenerator:
    ...
    def get_random_progression(self, genre, theme):
        ...
        # Choose a random progressions
            chosen = random.choice(rows)
            numerals = chosen['roman_numerals'].split(',') # roman_numerals is a comma-sep str : "i,III,iv,VI"
            return numerals
    ...
    def generate(self, genre, theme, key='C'):
        """
        ...
        chords = self.roman_to_chords(roman_list, key)
        return chords